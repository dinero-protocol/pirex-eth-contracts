...
import {DineroERC20RebaseUpgradeable} from "src/layer2/DineroERC20RebaseUpgradeable.sol";
...
import {L2SyncPool} from "src/layer2/old/L2SyncPool.sol";
...
import {IRateProvider} from "src/layer2/interfaces/IRateProvider.sol";
    ...
    PausableUpgradeable
{
    ...
    function initialize(
        ...
        __L2BaseSyncPool_init(_l2ExchangeRateProvider, _rateLimiter, _bridgeQuoter);
    ...
    function _lzReceive(
        ...
        if (_messageType == Constants.MESSAGE_TYPE_DEPOSIT) {
    ...
    /**
     * @param  id  bytes32  Sync id
     * @return Index.
     */
    function syncIdIndex(bytes32 id) external view returns (uint256) {
        return _getLiquidStakingTokenStorage().syncIdIndex[id];
    }
    ...
    /**
     * @return The unsynced pending deposit amount.
     */
    function unsyncedPendingDeposit() external view returns (uint256) {
        return _getLiquidStakingTokenStorage().unsyncedPendingDeposit;
    }

    /**
     * @return The synced pending deposit amount.
     */
    function syncedPendingDeposit() external view returns (uint256) {
        return _getLiquidStakingTokenStorage().syncedPendingDeposit;
    }
